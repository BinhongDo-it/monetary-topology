...
**`run_all.py` 2026-09-07 作废，而本条的判别式不受影响。**
那个脚本是「跑全部站 → 重渲 `RESULTS.md` → CI 比对」这条链的中间一环，
而三个端 2026-08-21 同日全部退役。**它的作业表覆盖 365 个脚本里的 52 个**
（25 站全覆盖、10 站部分、50 站零覆盖，A3 族、A7 族、C 轨全部在零覆盖那一格），
**`--touched` 与 `--skip-done` 随它一起作废，它们本来也只是那 52 个之内的便利。**
本条真正承重的是那条判别式（摘要里的数是读出来的还是现算的），它与哪个脚本实现它无关。
