...
the B8 inputs register §6.2.10. Reads the core table and the
...
form, and entry A23 of the project's objection cache (held outside this
repository) records its independent defence.
...
#: source (engineering rule 5). The per-loop bound is
#:
...
def find_clean_cures(c: K.Core, require_no_defer: bool = True):
    ...
    it is the 2026-08-17 correction (O28, the B8 inputs register §6.6.17).
...
def selftest() -> int:
    ...
    # number, which is the shape pit 32 keeps coming back in.
