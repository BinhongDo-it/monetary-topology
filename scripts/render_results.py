"""Regenerate RESULTS.md from ``results/``.

Two inputs, both under ``results/`` and both deterministic:

* ``*.json``  -- run records, rendered into criteria tables.
* ``*.section.md`` -- **written sections**, spliced in verbatim.
...
**That reason does not forbid prose, it forbids prose replacing the records.**
A written section sits beside the tables and cannot delete a failed criterion
out of them, so the guard survives while a conclusion that no record can carry
-- a reading held outside these files, a stage whose numbers are diagnostic --
still reaches the page. Write those into ``results/<name>.section.md``; do not
type into RESULTS.md, which is overwritten on the next run.
...
Generated by `scripts/render_results.py` from `results/`: the JSON run records,
and the written sections in `results/*.section.md`.

Do not edit this file by hand, because it is overwritten on the next run. To
write prose into it, add or edit a `results/*.section.md`.
...
committed and by whom, and a "last generated" line would make every rebuild a
change on content that is character-for-character identical.
"""
...
STAGE_TITLES = {
    ...
    # B10 reaches this file only through a written section: every one of its
    # sixty records is `diagnostic_only`, so the glob below skips all of them
    # and the stage would otherwise be absent from a file that lists every
    # other stage in the programme.
    "B10": (
        "B10 \u2014 the holonomy machinery rebuilt on a second carrier, and "
        "what that carrier stops recording in mid-2019"
    ),
...
def criteria_table(criteria: list[dict]) -> list[str]:
    ...
    for c in criteria:
        ...
        # The label, under either key. Three B15 records write `criterion`
        # where the rest write `name`, and until 2026-08-21 that killed the
        # whole render with a `KeyError` that named neither the file nor the
        # field. **The same defect this file already fixed once**: the
        # `b9_datasets.json` episode, whose lesson is written into
        # `is_record`. Tolerated and shown rather than raised, for the reason
        # the line above tolerates a missing detail.
        label = c.get("name") or c.get("criterion")
        lines.append(f"| {mark_of(c)} | "
                     f"{_cell(label or '_unnamed criterion_')} | {detail} |")
    return lines
...
#: A written section names its stage on the first line, in the same plain
#: uppercase-key form the station documents use, so it is greppable and a
#: reader can see where the section will land without running anything.
SECTION_KEY = "STAGE:"


def written_sections() -> dict[str, list[tuple[str, str]]]:
    """``results/*.section.md`` -> ``{stage: [(stem, body), ...]}``.

    Sorted by filename and read with the encoding pinned, so the output stays a
    pure function of what is on disk. A file whose first line is not
    ``STAGE: <name>`` is skipped and named, for the reason every other skip in
    this file is named: one that is silent is one nobody knows about.
    """
    out: dict[str, list[tuple[str, str]]] = {}
    bad: list[str] = []
    for path in sorted(RESULTS.glob("*.section.md")):
        text = path.read_text(encoding="utf-8")
        first, _, body = text.partition("\n")
        if not first.startswith(SECTION_KEY):
            bad.append(path.name)
            continue
        stage = first[len(SECTION_KEY):].strip()
        if not stage:
            bad.append(path.name)
            continue
        out.setdefault(stage, []).append(
            (path.name[:-len(".section.md")], body.strip("\n")))
    for name in bad:
        print(f"  written section without a `{SECTION_KEY} <name>` first "
              f"line, so not placed: {name}")
    return out


def main() -> int:
    records = sorted(RESULTS.glob("*.json"))
    written = written_sections()
    if not records:
    ...
    #: A stage that has only written sections still gets a heading, at the
    #: position its filename sorts to. Without this a stage whose every record
    #: is diagnostic could not reach the page at all, which is the case this
    #: channel exists for.
    for stage in written:
        groups.setdefault(stage, [])

    # **Insertion order, not sorted.** `records` is sorted and dicts keep
    # insertion order, so a stage sits at the position of its first record.
    # Sorting here instead would reorder the whole file on the first rebuild.
    for stage, items in groups.items():
        prose = written.get(stage, [])
        if len(items) == 1 and not prose:
    ...
    print(f"wrote {OUT.relative_to(ROOT)} from "
          f"{len(records) - len(off) - len(bare)} record(s) and "
          f"{sum(len(v) for v in written.values())} written section(s) "
